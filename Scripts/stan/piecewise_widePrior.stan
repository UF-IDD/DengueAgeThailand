functions {
    ...
    real[] expandParam( real[] params , int len_preTimes){
        int ntimes = size(params) + len_preTimes - 1;
        real outParams[ntimes];
        for (n in 1:ntimes){
            if (n <= len_preTimes) {
                outParams[n] = params[1];
            } else {
                outParams[n] = params[n - len_preTimes + 1];
            }
        }
        return outParams;
    }
...
parameters {
    ...
    // model parameters
    real<lower=0.0,upper=1.0> lambda[ len_times + 1 ];
...
#include /Scripts/stan/calcExpectedCaseCounts_piecewise.stan
    ...
    binnedQe[ ,1] ~ beta(2.0, 6.0);   // skewed towards 0.0, E(x)=0.25
    binnedQe[ ,2] ~ beta(6.0, 2.0);   // skewed towards 1.0, E(x)=0.75
    binnedQe[ ,3] ~ beta(2.0, 6.0);
    binnedQe[ ,4] ~ beta(2.0, 6.0);
